const restaurents=[
    {
      "images": "image_1 Umed Bhawan Palace",
      "name": "culinnary Haaven",
      "rating": 1,
      "food_types": "Dairy",
      "price_for_two": 855,
      "location": "Air Force Area",
      "distance_from_customer_house": 7.3,
      "offers": 17,
      "alcohol": false,
      "restaurent_open_time": 13,
      "restaurent_close_time": 1
    },
    {
      "images": "image_5 flamingo",
      "name": "Bazaar",
      "rating": 4,
      "food_types": "Seafood",
      "price_for_two": 825,
      "location": "Ratanada",
      "distance_from_customer_house": 8.6,
      "offers": 4,
      "alcohol": false,
      "restaurent_open_time": 11,
      "restaurent_close_time": 23
    },
    {
      "images": "image_2 1932 house kitchen and bar",
      "name": "ektara-radisson jodhpur",
      "rating": 3,
      "food_types": "Nuts",
      "price_for_two": 815,
      "location": "Sardarpura",
      "distance_from_customer_house": 1.5,
      "offers": 4,
      "alcohol": false,
      "restaurent_open_time": 7,
      "restaurent_close_time": 19
    },
    {
      "images": "image_3 khas bhag",
      "name": "Bazaar",
      "rating": 1,
      "food_types": "Seafood",
      "price_for_two": 507,
      "location": "Circuit House Road",
      "distance_from_customer_house": 2.6,
      "offers": 12,
      "alcohol": false,
      "restaurent_open_time": 23,
      "restaurent_close_time": 11
    },
    {
      "images": "image_8 the filos cafe and restaurent",
      "name": "the filos cafe and restaurent",
      "rating": 5,
      "food_types": "Meat",
      "price_for_two": 1275,
      "location": "Clock Tower Area",
      "distance_from_customer_house": 1.8,
      "offers": 13,
      "alcohol": false,
      "restaurent_open_time": 1,
      "restaurent_close_time": 13
    },
    {
      "images": "image_10 Bazaar",
      "name": "cosa blanca",
      "rating": 2,
      "food_types": "Meat",
      "price_for_two": 1866,
      "location": "Clock Tower Area",
      "distance_from_customer_house": 10.8,
      "offers": 1,
      "alcohol": false,
      "restaurent_open_time": 6,
      "restaurent_close_time": 18
    },
    {
      "images": "image_4 Royal Castle Restaurent",
      "name": "the filos cafe and restaurent",
      "rating": 1,
      "food_types": "Sweets",
      "price_for_two": 491,
      "location": "Circuit House Road",
      "distance_from_customer_house": 10.0,
      "offers": 20,
      "alcohol": false,
      "restaurent_open_time": 22,
      "restaurent_close_time": 10
    },
    {
      "images": "image_8 the filos cafe and restaurent",
      "name": "culinnary Haaven",
      "rating": 1,
      "food_types": "Sweets",
      "price_for_two": 1159,
      "location": "Residency Road",
      "distance_from_customer_house": 2.0,
      "offers": 23,
      "alcohol": false,
      "restaurent_open_time": 22,
      "restaurent_close_time": 10
    },
    {
      "images": "image_10 Bazaar",
      "name": "flamingo",
      "rating": 2,
      "food_types": "Seafood",
      "price_for_two": 1723,
      "location": "Clock Tower Area",
      "distance_from_customer_house": 6.2,
      "offers": 26,
      "alcohol": true,
      "restaurent_open_time": 17,
      "restaurent_close_time": 5
    },
    {
      "images": "image_7 ektara-radisson jodhpur",
      "name": "1932 house kitchen and bar",
      "rating": 1,
      "food_types": "Seafood",
      "price_for_two": 1259,
      "location": "Mandore Road",
      "distance_from_customer_house": 5.8,
      "offers": 18,
      "alcohol": false,
      "restaurent_open_time": 12,
      "restaurent_close_time": 0
    },
    {
      "images": "image_10 Bazaar",
      "name": "ektara-radisson jodhpur",
      "rating": 1,
      "food_types": "Sweets",
      "price_for_two": 773,
      "location": "Ratanada",
      "distance_from_customer_house": 8.0,
      "offers": 20,
      "alcohol": false,
      "restaurent_open_time": 14,
      "restaurent_close_time": 2
    },
    {
      "images": "image_5 flamingo",
      "name": "ektara-radisson jodhpur",
      "rating": 1,
      "food_types": "Grains",
      "price_for_two": 1756,
      "location": "Circuit House Road",
      "distance_from_customer_house": 8.0,
      "offers": 2,
      "alcohol": false,
      "restaurent_open_time": 17,
      "restaurent_close_time": 5
    },
    {
      "images": "image_5 flamingo",
      "name": "Umed Bhawan Palace",
      "rating": 2,
      "food_types": "Nuts",
      "price_for_two": 760,
      "location": "Ratanada",
      "distance_from_customer_house": 5.1,
      "offers": 15,
      "alcohol": false,
      "restaurent_open_time": 4,
      "restaurent_close_time": 16
    },
    {
      "images": "image_10 Bazaar",
      "name": "ektara-radisson jodhpur",
      "rating": 5,
      "food_types": "Vegetables",
      "price_for_two": 1703,
      "location": "Sardarpura",
      "distance_from_customer_house": 6.9,
      "offers": 3,
      "alcohol": false,
      "restaurent_open_time": 3,
      "restaurent_close_time": 15
    },
    {
      "images": "image_6 cosa blanca",
      "name": "ektara-radisson jodhpur",
      "rating": 2,
      "food_types": "Legumes",
      "price_for_two": 1029,
      "location": "Clock Tower Area",
      "distance_from_customer_house": 3.2,
      "offers": 11,
      "alcohol": false,
      "restaurent_open_time": 4,
      "restaurent_close_time": 16
    },
    {
      "images": "image_9 culinnary Haaven",
      "name": "khas bhag",
      "rating": 3,
      "food_types": "Beverages",
      "price_for_two": 2051,
      "location": "Sardarpura",
      "distance_from_customer_house": 5.2,
      "offers": 15,
      "alcohol": false,
      "restaurent_open_time": 10,
      "restaurent_close_time": 22
    },
    {
      "images": "image_3 khas bhag",
      "name": "cosa blanca",
      "rating": 3,
      "food_types": "Seafood",
      "price_for_two": 1672,
      "location": "Shastri Nagar",
      "distance_from_customer_house": 6.9,
      "offers": 19,
      "alcohol": false,
      "restaurent_open_time": 2,
      "restaurent_close_time": 14
    },
    {
      "images": "image_10 Bazaar",
      "name": "the filos cafe and restaurent",
      "rating": 4,
      "food_types": "Nuts",
      "price_for_two": 578,
      "location": "Ratanada",
      "distance_from_customer_house": 3.0,
      "offers": 9,
      "alcohol": true,
      "restaurent_open_time": 9,
      "restaurent_close_time": 21
    },
    {
      "images": "image_7 ektara-radisson jodhpur",
      "name": "khas bhag",
      "rating": 2,
      "food_types": "Legumes",
      "price_for_two": 1723,
      "location": "Ratanada",
      "distance_from_customer_house": 2.4,
      "offers": 19,
      "alcohol": false,
      "restaurent_open_time": 17,
      "restaurent_close_time": 5
    },
    {
      "images": "image_3 khas bhag",
      "name": "culinnary Haaven",
      "rating": 4,
      "food_types": "Legumes",
      "price_for_two": 2451,
      "location": "Clock Tower Area",
      "distance_from_customer_house": 9.2,
      "offers": 18,
      "alcohol": false,
      "restaurent_open_time": 13,
      "restaurent_close_time": 1
    },
    {
      "images": "image_3 khas bhag",
      "name": "Umed Bhawan Palace",
      "rating": 5,
      "food_types": "Nuts",
      "price_for_two": 1078,
      "location": "Residency Road",
      "distance_from_customer_house": 7.3,
      "offers": 24,
      "alcohol": true,
      "restaurent_open_time": 4,
      "restaurent_close_time": 16
    },
    {
      "images": "image_8 the filos cafe and restaurent",
      "name": "cosa blanca",
      "rating": 1,
      "food_types": "Grains",
      "price_for_two": 877,
      "location": "Circuit House Road",
      "distance_from_customer_house": 10.4,
      "offers": 1,
      "alcohol": false,
      "restaurent_open_time": 19,
      "restaurent_close_time": 7
    },
    {
      "images": "image_8 the filos cafe and restaurent",
      "name": "flamingo",
      "rating": 2,
      "food_types": "Dairy",
      "price_for_two": 272,
      "location": "Circuit House Road",
      "distance_from_customer_house": 8.0,
      "offers": 9,
      "alcohol": false,
      "restaurent_open_time": 20,
      "restaurent_close_time": 8
    },
    {
      "images": "image_8 the filos cafe and restaurent",
      "name": "Bazaar",
      "rating": 4,
      "food_types": "Legumes",
      "price_for_two": 2250,
      "location": "Mandore Road",
      "distance_from_customer_house": 9.2,
      "offers": 26,
      "alcohol": false,
      "restaurent_open_time": 19,
      "restaurent_close_time": 7
    },
    {
      "images": "image_4 Royal Castle Restaurent",
      "name": "cosa blanca",
      "rating": 3,
      "food_types": "Meat",
      "price_for_two": 1942,
      "location": "Clock Tower Area",
      "distance_from_customer_house": 7.4,
      "offers": 2,
      "alcohol": false,
      "restaurent_open_time": 12,
      "restaurent_close_time": 0
    },
    {
      "images": "image_9 culinnary Haaven",
      "name": "Royal Castle Restaurent",
      "rating": 1,
      "food_types": "Fruits",
      "price_for_two": 278,
      "location": "Panch Batti Circle",
      "distance_from_customer_house": 8.3,
      "offers": 2,
      "alcohol": true,
      "restaurent_open_time": 0,
      "restaurent_close_time": 12
    },
    {
      "images": "image_1 Umed Bhawan Palace",
      "name": "Royal Castle Restaurent",
      "rating": 5,
      "food_types": "Fruits",
      "price_for_two": 682,
      "location": "Clock Tower Area",
      "distance_from_customer_house": 7.6,
      "offers": 0,
      "alcohol": false,
      "restaurent_open_time": 10,
      "restaurent_close_time": 22
    },
    {
      "images": "image_8 the filos cafe and restaurent",
      "name": "flamingo",
      "rating": 4,
      "food_types": "Dairy",
      "price_for_two": 1768,
      "location": "Panch Batti Circle",
      "distance_from_customer_house": 6.0,
      "offers": 24,
      "alcohol": false,
      "restaurent_open_time": 13,
      "restaurent_close_time": 1
    },
    {
      "images": "image_7 ektara-radisson jodhpur",
      "name": "Umed Bhawan Palace",
      "rating": 1,
      "food_types": "Beverages",
      "price_for_two": 2258,
      "location": "Circuit House Road",
      "distance_from_customer_house": 1.3,
      "offers": 9,
      "alcohol": false,
      "restaurent_open_time": 16,
      "restaurent_close_time": 4
    },
    {
      "images": "image_8 the filos cafe and restaurent",
      "name": "flamingo",
      "rating": 1,
      "food_types": "Seafood",
      "price_for_two": 2401,
      "location": "Circuit House Road",
      "distance_from_customer_house": 4.7,
      "offers": 3,
      "alcohol": false,
      "restaurent_open_time": 18,
      "restaurent_close_time": 6
    },
    {
      "images": "image_5 flamingo",
      "name": "ektara-radisson jodhpur",
      "rating": 1,
      "food_types": "Seafood",
      "price_for_two": 196,
      "location": "Sardarpura",
      "distance_from_customer_house": 8.4,
      "offers": 12,
      "alcohol": false,
      "restaurent_open_time": 3,
      "restaurent_close_time": 15
    },
    {
      "images": "image_8 the filos cafe and restaurent",
      "name": "flamingo",
      "rating": 1,
      "food_types": "Fruits",
      "price_for_two": 206,
      "location": "Air Force Area",
      "distance_from_customer_house": 10.8,
      "offers": 26,
      "alcohol": true,
      "restaurent_open_time": 14,
      "restaurent_close_time": 2
    },
    {
      "images": "image_10 Bazaar",
      "name": "culinnary Haaven",
      "rating": 3,
      "food_types": "Grains",
      "price_for_two": 2440,
      "location": "Residency Road",
      "distance_from_customer_house": 4.8,
      "offers": 24,
      "alcohol": true,
      "restaurent_open_time": 10,
      "restaurent_close_time": 22
    },
    {
      "images": "image_1 Umed Bhawan Palace",
      "name": "Royal Castle Restaurent",
      "rating": 4,
      "food_types": "Beverages",
      "price_for_two": 1154,
      "location": "Ratanada",
      "distance_from_customer_house": 3.6,
      "offers": 17,
      "alcohol": false,
      "restaurent_open_time": 13,
      "restaurent_close_time": 1
    },
    {
      "images": "image_1 Umed Bhawan Palace",
      "name": "Royal Castle Restaurent",
      "rating": 5,
      "food_types": "Vegetables",
      "price_for_two": 2084,
      "location": "Sardarpura",
      "distance_from_customer_house": 4.1,
      "offers": 17,
      "alcohol": false,
      "restaurent_open_time": 10,
      "restaurent_close_time": 22
    },
    {
      "images": "image_1 Umed Bhawan Palace",
      "name": "culinnary Haaven",
      "rating": 2,
      "food_types": "Sweets",
      "price_for_two": 339,
      "location": "Residency Road",
      "distance_from_customer_house": 10.3,
      "offers": 4,
      "alcohol": true,
      "restaurent_open_time": 7,
      "restaurent_close_time": 19
    },
    {
      "images": "image_6 cosa blanca",
      "name": "Umed Bhawan Palace",
      "rating": 5,
      "food_types": "Fruits",
      "price_for_two": 2393,
      "location": "Shastri Nagar",
      "distance_from_customer_house": 2.6,
      "offers": 2,
      "alcohol": true,
      "restaurent_open_time": 19,
      "restaurent_close_time": 7
    },
    {
      "images": "image_3 khas bhag",
      "name": "the filos cafe and restaurent",
      "rating": 4,
      "food_types": "Grains",
      "price_for_two": 1422,
      "location": "Circuit House Road",
      "distance_from_customer_house": 10.0,
      "offers": 16,
      "alcohol": false,
      "restaurent_open_time": 13,
      "restaurent_close_time": 1
    },
    {
      "images": "image_7 ektara-radisson jodhpur",
      "name": "culinnary Haaven",
      "rating": 2,
      "food_types": "Beverages",
      "price_for_two": 1134,
      "location": "Panch Batti Circle",
      "distance_from_customer_house": 8.2,
      "offers": 29,
      "alcohol": false,
      "restaurent_open_time": 12,
      "restaurent_close_time": 0
    },
    {
      "images": "image_10 Bazaar",
      "name": "ektara-radisson jodhpur",
      "rating": 5,
      "food_types": "Sweets",
      "price_for_two": 1131,
      "location": "Sardarpura",
      "distance_from_customer_house": 1.8,
      "offers": 12,
      "alcohol": true,
      "restaurent_open_time": 19,
      "restaurent_close_time": 7
    },
    {
      "images": "image_4 Royal Castle Restaurent",
      "name": "Umed Bhawan Palace",
      "rating": 4,
      "food_types": "Seafood",
      "price_for_two": 1790,
      "location": "Circuit House Road",
      "distance_from_customer_house": 5.4,
      "offers": 4,
      "alcohol": false,
      "restaurent_open_time": 23,
      "restaurent_close_time": 11
    },
    {
      "images": "image_4 Royal Castle Restaurent",
      "name": "the filos cafe and restaurent",
      "rating": 4,
      "food_types": "Seafood",
      "price_for_two": 224,
      "location": "Clock Tower Area",
      "distance_from_customer_house": 6.9,
      "offers": 8,
      "alcohol": false,
      "restaurent_open_time": 21,
      "restaurent_close_time": 9
    },
    {
      "images": "image_7 ektara-radisson jodhpur",
      "name": "1932 house kitchen and bar",
      "rating": 2,
      "food_types": "Grains",
      "price_for_two": 488,
      "location": "Cantt Area",
      "distance_from_customer_house": 3.6,
      "offers": 9,
      "alcohol": true,
      "restaurent_open_time": 1,
      "restaurent_close_time": 13
    },
    {
      "images": "image_3 khas bhag",
      "name": "ektara-radisson jodhpur",
      "rating": 4,
      "food_types": "Vegetables",
      "price_for_two": 190,
      "location": "Circuit House Road",
      "distance_from_customer_house": 3.0,
      "offers": 23,
      "alcohol": false,
      "restaurent_open_time": 4,
      "restaurent_close_time": 16
    },
    {
      "images": "image_5 flamingo",
      "name": "flamingo",
      "rating": 2,
      "food_types": "Meat",
      "price_for_two": 511,
      "location": "Shastri Nagar",
      "distance_from_customer_house": 7.0,
      "offers": 6,
      "alcohol": false,
      "restaurent_open_time": 16,
      "restaurent_close_time": 4
    },
    {
      "images": "image_3 khas bhag",
      "name": "the filos cafe and restaurent",
      "rating": 5,
      "food_types": "Sweets",
      "price_for_two": 336,
      "location": "Air Force Area",
      "distance_from_customer_house": 4.9,
      "offers": 29,
      "alcohol": false,
      "restaurent_open_time": 18,
      "restaurent_close_time": 6
    },
    {
      "images": "image_6 cosa blanca",
      "name": "Bazaar",
      "rating": 1,
      "food_types": "Legumes",
      "price_for_two": 534,
      "location": "Sardarpura",
      "distance_from_customer_house": 7.9,
      "offers": 25,
      "alcohol": true,
      "restaurent_open_time": 12,
      "restaurent_close_time": 0
    },
    {
      "images": "image_5 flamingo",
      "name": "Umed Bhawan Palace",
      "rating": 2,
      "food_types": "Dairy",
      "price_for_two": 1658,
      "location": "Ratanada",
      "distance_from_customer_house": 1.0,
      "offers": 21,
      "alcohol": true,
      "restaurent_open_time": 2,
      "restaurent_close_time": 14
    },
    {
      "images": "image_10 Bazaar",
      "name": "the filos cafe and restaurent",
      "rating": 1,
      "food_types": "Grains",
      "price_for_two": 701,
      "location": "Panch Batti Circle",
      "distance_from_customer_house": 7.2,
      "offers": 27,
      "alcohol": true,
      "restaurent_open_time": 15,
      "restaurent_close_time": 3
    },
    {
      "images": "image_4 Royal Castle Restaurent",
      "name": "khas bhag",
      "rating": 4,
      "food_types": "Fruits",
      "price_for_two": 840,
      "location": "Mandore Road",
      "distance_from_customer_house": 6.2,
      "offers": 27,
      "alcohol": false,
      "restaurent_open_time": 1,
      "restaurent_close_time": 13
    },
    {
      "images": "image_5 flamingo",
      "name": "cosa blanca",
      "rating": 5,
      "food_types": "Dairy",
      "price_for_two": 1869,
      "location": "Air Force Area",
      "distance_from_customer_house": 5.7,
      "offers": 4,
      "alcohol": false,
      "restaurent_open_time": 0,
      "restaurent_close_time": 12
    },
    {
      "images": "image_8 the filos cafe and restaurent",
      "name": "flamingo",
      "rating": 4,
      "food_types": "Dairy",
      "price_for_two": 246,
      "location": "Shastri Nagar",
      "distance_from_customer_house": 5.1,
      "offers": 19,
      "alcohol": false,
      "restaurent_open_time": 3,
      "restaurent_close_time": 15
    },
    {
      "images": "image_7 ektara-radisson jodhpur",
      "name": "cosa blanca",
      "rating": 3,
      "food_types": "Beverages",
      "price_for_two": 808,
      "location": "Ratanada",
      "distance_from_customer_house": 8.2,
      "offers": 25,
      "alcohol": false,
      "restaurent_open_time": 17,
      "restaurent_close_time": 5
    },
    {
      "images": "image_1 Umed Bhawan Palace",
      "name": "the filos cafe and restaurent",
      "rating": 5,
      "food_types": "Legumes",
      "price_for_two": 601,
      "location": "Air Force Area",
      "distance_from_customer_house": 6.1,
      "offers": 29,
      "alcohol": false,
      "restaurent_open_time": 15,
      "restaurent_close_time": 3
    },
    {
      "images": "image_7 ektara-radisson jodhpur",
      "name": "Umed Bhawan Palace",
      "rating": 3,
      "food_types": "Seafood",
      "price_for_two": 869,
      "location": "Air Force Area",
      "distance_from_customer_house": 5.1,
      "offers": 10,
      "alcohol": false,
      "restaurent_open_time": 16,
      "restaurent_close_time": 4
    },
    {
      "images": "image_4 Royal Castle Restaurent",
      "name": "cosa blanca",
      "rating": 3,
      "food_types": "Meat",
      "price_for_two": 718,
      "location": "Residency Road",
      "distance_from_customer_house": 8.1,
      "offers": 29,
      "alcohol": false,
      "restaurent_open_time": 21,
      "restaurent_close_time": 9
    },
    {
      "images": "image_3 khas bhag",
      "name": "flamingo",
      "rating": 1,
      "food_types": "Seafood",
      "price_for_two": 1009,
      "location": "Ratanada",
      "distance_from_customer_house": 4.6,
      "offers": 10,
      "alcohol": true,
      "restaurent_open_time": 23,
      "restaurent_close_time": 11
    },
    {
      "images": "image_3 khas bhag",
      "name": "Bazaar",
      "rating": 4,
      "food_types": "Meat",
      "price_for_two": 1872,
      "location": "Cantt Area",
      "distance_from_customer_house": 8.2,
      "offers": 16,
      "alcohol": true,
      "restaurent_open_time": 0,
      "restaurent_close_time": 12
    },
    {
      "images": "image_8 the filos cafe and restaurent",
      "name": "1932 house kitchen and bar",
      "rating": 2,
      "food_types": "Seafood",
      "price_for_two": 2471,
      "location": "Mandore Road",
      "distance_from_customer_house": 5.7,
      "offers": 25,
      "alcohol": false,
      "restaurent_open_time": 13,
      "restaurent_close_time": 1
    },
    {
      "images": "image_3 khas bhag",
      "name": "flamingo",
      "rating": 4,
      "food_types": "Seafood",
      "price_for_two": 962,
      "location": "Circuit House Road",
      "distance_from_customer_house": 2.0,
      "offers": 26,
      "alcohol": true,
      "restaurent_open_time": 18,
      "restaurent_close_time": 6
    },
    {
      "images": "image_2 1932 house kitchen and bar",
      "name": "Royal Castle Restaurent",
      "rating": 4,
      "food_types": "Seafood",
      "price_for_two": 2361,
      "location": "Residency Road",
      "distance_from_customer_house": 2.1,
      "offers": 20,
      "alcohol": false,
      "restaurent_open_time": 17,
      "restaurent_close_time": 5
    },
    {
      "images": "image_9 culinnary Haaven",
      "name": "Royal Castle Restaurent",
      "rating": 1,
      "food_types": "Grains",
      "price_for_two": 1393,
      "location": "Mandore Road",
      "distance_from_customer_house": 8.7,
      "offers": 15,
      "alcohol": false,
      "restaurent_open_time": 1,
      "restaurent_close_time": 13
    },
    {
      "images": "image_2 1932 house kitchen and bar",
      "name": "Royal Castle Restaurent",
      "rating": 5,
      "food_types": "Legumes",
      "price_for_two": 2487,
      "location": "Circuit House Road",
      "distance_from_customer_house": 6.4,
      "offers": 25,
      "alcohol": false,
      "restaurent_open_time": 16,
      "restaurent_close_time": 4
    },
    {
      "images": "image_7 ektara-radisson jodhpur",
      "name": "the filos cafe and restaurent",
      "rating": 4,
      "food_types": "Beverages",
      "price_for_two": 1283,
      "location": "Circuit House Road",
      "distance_from_customer_house": 1.7,
      "offers": 16,
      "alcohol": true,
      "restaurent_open_time": 11,
      "restaurent_close_time": 23
    },
    {
      "images": "image_8 the filos cafe and restaurent",
      "name": "cosa blanca",
      "rating": 3,
      "food_types": "Beverages",
      "price_for_two": 2112,
      "location": "Clock Tower Area",
      "distance_from_customer_house": 10.1,
      "offers": 9,
      "alcohol": true,
      "restaurent_open_time": 7,
      "restaurent_close_time": 19
    },
    {
      "images": "image_7 ektara-radisson jodhpur",
      "name": "Umed Bhawan Palace",
      "rating": 3,
      "food_types": "Sweets",
      "price_for_two": 1521,
      "location": "Shastri Nagar",
      "distance_from_customer_house": 2.3,
      "offers": 14,
      "alcohol": true,
      "restaurent_open_time": 14,
      "restaurent_close_time": 2
    },
    {
      "images": "image_5 flamingo",
      "name": "ektara-radisson jodhpur",
      "rating": 1,
      "food_types": "Seafood",
      "price_for_two": 1423,
      "location": "Mandore Road",
      "distance_from_customer_house": 1.4,
      "offers": 21,
      "alcohol": true,
      "restaurent_open_time": 1,
      "restaurent_close_time": 13
    },
    {
      "images": "image_6 cosa blanca",
      "name": "the filos cafe and restaurent",
      "rating": 1,
      "food_types": "Nuts",
      "price_for_two": 1442,
      "location": "Residency Road",
      "distance_from_customer_house": 5.3,
      "offers": 6,
      "alcohol": false,
      "restaurent_open_time": 1,
      "restaurent_close_time": 13
    },
    {
      "images": "image_2 1932 house kitchen and bar",
      "name": "flamingo",
      "rating": 1,
      "food_types": "Fruits",
      "price_for_two": 970,
      "location": "Ratanada",
      "distance_from_customer_house": 6.4,
      "offers": 17,
      "alcohol": false,
      "restaurent_open_time": 8,
      "restaurent_close_time": 20
    },
    {
      "images": "image_6 cosa blanca",
      "name": "khas bhag",
      "rating": 2,
      "food_types": "Meat",
      "price_for_two": 2330,
      "location": "Clock Tower Area",
      "distance_from_customer_house": 3.6,
      "offers": 12,
      "alcohol": true,
      "restaurent_open_time": 17,
      "restaurent_close_time": 5
    },
    {
      "images": "image_2 1932 house kitchen and bar",
      "name": "ektara-radisson jodhpur",
      "rating": 5,
      "food_types": "Fruits",
      "price_for_two": 898,
      "location": "Shastri Nagar",
      "distance_from_customer_house": 10.8,
      "offers": 9,
      "alcohol": false,
      "restaurent_open_time": 1,
      "restaurent_close_time": 13
    },
    {
      "images": "image_1 Umed Bhawan Palace",
      "name": "Royal Castle Restaurent",
      "rating": 5,
      "food_types": "Legumes",
      "price_for_two": 1661,
      "location": "Sardarpura",
      "distance_from_customer_house": 7.7,
      "offers": 21,
      "alcohol": false,
      "restaurent_open_time": 19,
      "restaurent_close_time": 7
    },
    {
      "images": "image_10 Bazaar",
      "name": "flamingo",
      "rating": 5,
      "food_types": "Beverages",
      "price_for_two": 1420,
      "location": "Panch Batti Circle",
      "distance_from_customer_house": 6.2,
      "offers": 27,
      "alcohol": false,
      "restaurent_open_time": 6,
      "restaurent_close_time": 18
    },
    {
      "images": "image_10 Bazaar",
      "name": "Royal Castle Restaurent",
      "rating": 2,
      "food_types": "Beverages",
      "price_for_two": 335,
      "location": "Ratanada",
      "distance_from_customer_house": 8.8,
      "offers": 12,
      "alcohol": false,
      "restaurent_open_time": 0,
      "restaurent_close_time": 12
    },
    {
      "images": "image_6 cosa blanca",
      "name": "flamingo",
      "rating": 3,
      "food_types": "Fruits",
      "price_for_two": 1724,
      "location": "Circuit House Road",
      "distance_from_customer_house": 3.4,
      "offers": 25,
      "alcohol": true,
      "restaurent_open_time": 7,
      "restaurent_close_time": 19
    },
    {
      "images": "image_4 Royal Castle Restaurent",
      "name": "khas bhag",
      "rating": 4,
      "food_types": "Legumes",
      "price_for_two": 2343,
      "location": "Cantt Area",
      "distance_from_customer_house": 5.8,
      "offers": 8,
      "alcohol": false,
      "restaurent_open_time": 17,
      "restaurent_close_time": 5
    },
    {
      "images": "image_7 ektara-radisson jodhpur",
      "name": "culinnary Haaven",
      "rating": 4,
      "food_types": "Sweets",
      "price_for_two": 1134,
      "location": "Residency Road",
      "distance_from_customer_house": 3.1,
      "offers": 4,
      "alcohol": true,
      "restaurent_open_time": 22,
      "restaurent_close_time": 10
    },
    {
      "images": "image_1 Umed Bhawan Palace",
      "name": "culinnary Haaven",
      "rating": 1,
      "food_types": "Meat",
      "price_for_two": 1803,
      "location": "Residency Road",
      "distance_from_customer_house": 5.3,
      "offers": 8,
      "alcohol": false,
      "restaurent_open_time": 4,
      "restaurent_close_time": 16
    },
    {
      "images": "image_5 flamingo",
      "name": "khas bhag",
      "rating": 3,
      "food_types": "Beverages",
      "price_for_two": 2200,
      "location": "Mandore Road",
      "distance_from_customer_house": 7.1,
      "offers": 4,
      "alcohol": false,
      "restaurent_open_time": 20,
      "restaurent_close_time": 8
    },
    {
      "images": "image_2 1932 house kitchen and bar",
      "name": "Umed Bhawan Palace",
      "rating": 3,
      "food_types": "Beverages",
      "price_for_two": 1099,
      "location": "Panch Batti Circle",
      "distance_from_customer_house": 3.0,
      "offers": 3,
      "alcohol": false,
      "restaurent_open_time": 9,
      "restaurent_close_time": 21
    },
    {
      "images": "image_8 the filos cafe and restaurent",
      "name": "1932 house kitchen and bar",
      "rating": 3,
      "food_types": "Grains",
      "price_for_two": 543,
      "location": "Ratanada",
      "distance_from_customer_house": 5.3,
      "offers": 20,
      "alcohol": false,
      "restaurent_open_time": 22,
      "restaurent_close_time": 10
    },
    {
      "images": "image_10 Bazaar",
      "name": "Royal Castle Restaurent",
      "rating": 5,
      "food_types": "Dairy",
      "price_for_two": 344,
      "location": "Ratanada",
      "distance_from_customer_house": 8.4,
      "offers": 15,
      "alcohol": false,
      "restaurent_open_time": 23,
      "restaurent_close_time": 11
    },
    {
      "images": "image_3 khas bhag",
      "name": "khas bhag",
      "rating": 1,
      "food_types": "Meat",
      "price_for_two": 1306,
      "location": "Panch Batti Circle",
      "distance_from_customer_house": 10.1,
      "offers": 5,
      "alcohol": false,
      "restaurent_open_time": 9,
      "restaurent_close_time": 21
    },
    {
      "images": "image_1 Umed Bhawan Palace",
      "name": "Umed Bhawan Palace",
      "rating": 3,
      "food_types": "Fruits",
      "price_for_two": 2064,
      "location": "Clock Tower Area",
      "distance_from_customer_house": 5.2,
      "offers": 10,
      "alcohol": false,
      "restaurent_open_time": 20,
      "restaurent_close_time": 8
    },
    {
      "images": "image_7 ektara-radisson jodhpur",
      "name": "Bazaar",
      "rating": 5,
      "food_types": "Beverages",
      "price_for_two": 1480,
      "location": "Residency Road",
      "distance_from_customer_house": 5.5,
      "offers": 13,
      "alcohol": false,
      "restaurent_open_time": 22,
      "restaurent_close_time": 10
    },
    {
      "images": "image_6 cosa blanca",
      "name": "culinnary Haaven",
      "rating": 5,
      "food_types": "Fruits",
      "price_for_two": 514,
      "location": "Ratanada",
      "distance_from_customer_house": 10.3,
      "offers": 4,
      "alcohol": true,
      "restaurent_open_time": 17,
      "restaurent_close_time": 5
    },
    {
      "images": "image_4 Royal Castle Restaurent",
      "name": "Umed Bhawan Palace",
      "rating": 5,
      "food_types": "Legumes",
      "price_for_two": 2300,
      "location": "Mandore Road",
      "distance_from_customer_house": 4.4,
      "offers": 4,
      "alcohol": true,
      "restaurent_open_time": 20,
      "restaurent_close_time": 8
    },
    {
      "images": "image_2 1932 house kitchen and bar",
      "name": "Bazaar",
      "rating": 4,
      "food_types": "Sweets",
      "price_for_two": 487,
      "location": "Residency Road",
      "distance_from_customer_house": 10.9,
      "offers": 18,
      "alcohol": false,
      "restaurent_open_time": 21,
      "restaurent_close_time": 9
    },
    {
      "images": "image_2 1932 house kitchen and bar",
      "name": "ektara-radisson jodhpur",
      "rating": 2,
      "food_types": "Dairy",
      "price_for_two": 2073,
      "location": "Sardarpura",
      "distance_from_customer_house": 3.9,
      "offers": 1,
      "alcohol": true,
      "restaurent_open_time": 7,
      "restaurent_close_time": 19
    },
    {
      "images": "image_5 flamingo",
      "name": "flamingo",
      "rating": 3,
      "food_types": "Fruits",
      "price_for_two": 416,
      "location": "Mandore Road",
      "distance_from_customer_house": 7.6,
      "offers": 16,
      "alcohol": false,
      "restaurent_open_time": 0,
      "restaurent_close_time": 12
    },
    {
      "images": "image_9 culinnary Haaven",
      "name": "cosa blanca",
      "rating": 4,
      "food_types": "Legumes",
      "price_for_two": 652,
      "location": "Panch Batti Circle",
      "distance_from_customer_house": 3.0,
      "offers": 19,
      "alcohol": false,
      "restaurent_open_time": 17,
      "restaurent_close_time": 5
    },
    {
      "images": "image_1 Umed Bhawan Palace",
      "name": "ektara-radisson jodhpur",
      "rating": 1,
      "food_types": "Beverages",
      "price_for_two": 653,
      "location": "Residency Road",
      "distance_from_customer_house": 1.1,
      "offers": 22,
      "alcohol": false,
      "restaurent_open_time": 11,
      "restaurent_close_time": 23
    },
    {
      "images": "image_9 culinnary Haaven",
      "name": "Royal Castle Restaurent",
      "rating": 3,
      "food_types": "Sweets",
      "price_for_two": 1175,
      "location": "Sardarpura",
      "distance_from_customer_house": 6.1,
      "offers": 16,
      "alcohol": true,
      "restaurent_open_time": 10,
      "restaurent_close_time": 22
    },
    {
      "images": "image_1 Umed Bhawan Palace",
      "name": "the filos cafe and restaurent",
      "rating": 4,
      "food_types": "Seafood",
      "price_for_two": 1367,
      "location": "Cantt Area",
      "distance_from_customer_house": 10.9,
      "offers": 16,
      "alcohol": false,
      "restaurent_open_time": 14,
      "restaurent_close_time": 2
    },
    {
      "images": "image_1 Umed Bhawan Palace",
      "name": "Umed Bhawan Palace",
      "rating": 3,
      "food_types": "Vegetables",
      "price_for_two": 170,
      "location": "Ratanada",
      "distance_from_customer_house": 7.4,
      "offers": 21,
      "alcohol": false,
      "restaurent_open_time": 5,
      "restaurent_close_time": 17
    },
    {
      "images": "image_2 1932 house kitchen and bar",
      "name": "khas bhag",
      "rating": 4,
      "food_types": "Nuts",
      "price_for_two": 1411,
      "location": "Mandore Road",
      "distance_from_customer_house": 2.7,
      "offers": 12,
      "alcohol": true,
      "restaurent_open_time": 3,
      "restaurent_close_time": 15
    },
    {
      "images": "image_5 flamingo",
      "name": "Royal Castle Restaurent",
      "rating": 1,
      "food_types": "Vegetables",
      "price_for_two": 2171,
      "location": "Residency Road",
      "distance_from_customer_house": 3.4,
      "offers": 28,
      "alcohol": true,
      "restaurent_open_time": 5,
      "restaurent_close_time": 17
    },
    {
      "images": "image_7 ektara-radisson jodhpur",
      "name": "flamingo",
      "rating": 5,
      "food_types": "Sweets",
      "price_for_two": 487,
      "location": "Panch Batti Circle",
      "distance_from_customer_house": 3.5,
      "offers": 6,
      "alcohol": false,
      "restaurent_open_time": 4,
      "restaurent_close_time": 16
    },
    {
      "images": "image_3 khas bhag",
      "name": "the filos cafe and restaurent",
      "rating": 2,
      "food_types": "Sweets",
      "price_for_two": 1774,
      "location": "Sardarpura",
      "distance_from_customer_house": 3.8,
      "offers": 29,
      "alcohol": false,
      "restaurent_open_time": 4,
      "restaurent_close_time": 16
    },
    {
      "images": "image_2 1932 house kitchen and bar",
      "name": "cosa blanca",
      "rating": 1,
      "food_types": "Grains",
      "price_for_two": 720,
      "location": "Clock Tower Area",
      "distance_from_customer_house": 10.1,
      "offers": 21,
      "alcohol": false,
      "restaurent_open_time": 2,
      "restaurent_close_time": 14
    }
  ]

  function getrestaurents(restaurents){
    const root = document.getElementById('root');

    restaurents.forEach(restaurent=>{

       
        // Create a card
        // 1: Image
        // 2: Card_content
        // i: Card_header (Name and rating)
        // ii: Card_footer (Food_type and price)
        // iii: card_location (Restaurant location, distance)


      // create a card
      const card = document.createElement('div');
      card.classList.add('card');


      // create Image
      const image = document.createElement("img");
      image.src=`images/${restaurent.images}.avif`;


      // Card_content
      const card_content=document.createElement('div');
      card_content.classList.add('card-content');


          //  card_header(name & rating)
          const card_header=document.createElement('div');
          card_header.classList.add('card-header');

          const h3=document.createElement('h3');
          h3.textContent=restaurent.name;

          const rate=document.createElement('span');
          rate.textContent="Rating: " +restaurent.rating;
          rate.classList.add('rating');

          card_header.appendChild(h3);
          card_header.appendChild(rate);




          //  card_footer(foodtype & price)
          const card_footer=document.createElement('div');
          card_footer.classList.add('card-footer');

          const food=document.createElement('span');
          food.textContent=restaurent.food_types;

          const price=document.createElement('span');
          price.textContent="₹"+restaurent.price_for_two;

          card_footer.appendChild(food);
          card_footer.appendChild(price);




          //  card_location(restaurent & dist)
          const card_location=document.createElement('div');
          card_location.classList.add('card-location');

          const location=document.createElement('span');
          location.textContent=restaurent.location;

          const distance=document.createElement('span');
          distance.textContent=restaurent.distance_from_customer_house+"km";

          card_location.appendChild(location);
          card_location.appendChild(distance);




          card_content.appendChild(card_header);
          card_content.appendChild(card_footer);
          card_content.appendChild(card_location);


          card.appendChild(image);
          card.appendChild(card_content);

          root.appendChild(card);

    })
  }
  getrestaurents(restaurents);


  document.getElementById("Alcohol").addEventListener('click',()=>{

    const result=restaurents.filter((obj)=>obj.alcohol);
    document.getElementById('root').replaceChildren();;
    getrestaurents(result);
  })

  document.getElementById("Rating").addEventListener('click',()=>{

    const result=restaurents.filter((obj)=>obj.rating>4.5);
    document.getElementById('root').replaceChildren();
    getrestaurents(result);
  })

  document.getElementById("Filters").addEventListener('click',()=>{
    document.getElementById('filterPopup').classList.remove("hidden");
  })

  document.getElementById('applyFilter').addEventListener('click',()=>{
    
    const elem=document.querySelector('input[name="filterOption"]:checked');
    const answer=elem.value;

    if(answer==="rating"){
      restaurents.sort((a,b)=>b.rating-a.rating);
    }
    else if(answer==="highLow"){
      restaurents.sort((a,b)=>b.price_for_two-a.price_for_two);
    }
    else if(answer==="costLowHigh"){
      restaurents.sort((a,b)=>a.price_for_two-b.price_for_two);
    }
    else if(answer==="distance"){
      restaurents.sort((a,b)=>a.distance_from_customer_house-b.distance_from_customer_house);
      
    }
    document.getElementById('root').replaceChildren();
    document.getElementById('filterPopup').classList.add("hidden");
    getrestaurents(restaurents);
  })